...
public class DemonicGorillaPlugin extends Plugin
{
                ...
                else if (animationId == AnimationID.DEMONIC_GORILLA_AOE_ATTACK && interacting != null)
                {
                    ...
                    else
                    {
                        if (tickCounter >= gorilla.getNextAttackTick())
                        {
                            ...
                            if (projectileId == GraphicID.DEMONIC_GORILLA_MAGIC)
                            {
                                onGorillaAttack(gorilla, DemonicGorilla.AttackStyle.MAGIC);
                            }
                            else if (projectileId == GraphicID.DEMONIC_GORILLA_RANGED)
                            {
                                onGorillaAttack(gorilla, DemonicGorilla.AttackStyle.RANGED);
                            }
                            else if (mp != null)
    ...
    public void onProjectileMoved(ProjectileMoved event)
    {
        ...
        if (projectileId != GraphicID.DEMONIC_GORILLA_RANGED &&
            projectileId != GraphicID.DEMONIC_GORILLA_MAGIC &&
            projectileId != GraphicID.DEMONIC_GORILLA_BOULDER)
        {
            return;
        }
        ...
        if (projectileId == GraphicID.DEMONIC_GORILLA_BOULDER)
        {
            recentBoulders.add(WorldPoint.fromLocal(client, event.getPosition()));
        }
        else if (projectileId == GraphicID.DEMONIC_GORILLA_MAGIC ||
            projectileId == GraphicID.DEMONIC_GORILLA_RANGED)
        {
        ...
        while (it.hasNext())
        {
            ...
            if (tickCounter >= attack.getFinishesOnTick())
            {
                ...
                else if (target.getRecentHitsplats().isEmpty())
                {
                    ...
                    .anyMatch(x -> x.getHitsplatType() == HitsplatID.BLOCK_ME || x.getHitsplatType() == HitsplatID.BLOCK_OTHER))
                {
